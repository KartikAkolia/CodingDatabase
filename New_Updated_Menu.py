class menu:
    ...
    def get_tablename(self, game):
        ...
        cursor.execute("SELECT NAME FROM Inventory WHERE ID=" + str(counter))
        result = cursor.fetchone()
        cursor.close()
        return result[0]

    def add_score(self, game):
        tablename = self.get_tablename(game)
        n = input("Enter The Player Name")
        s = input("Enter The Score")
        cursor = self.mydb.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM " + tablename + " WHERE CODE='" + n[0:1] + "'"
        )
        result = cursor.fetchone()
        if result[0] > 0:
    ...
    def revert(self, game):
        ...
        cursor = self.mydb.cursor()
        cursor.execute(query)
        query = "UPDATE " + tablename + " SET SCORE=0"
        print(query)
        cursor = self.mydb.cursor()
        cursor.execute(query)
        self.mydb.commit()
    ...
    def Service_Status(self):
        ...
        self.mydb.commit()
        cursor.close()
        query = self.os.system(
            "/etc/init.d/ssh status | grep -i running 1>/dev/null 2>/dev/null"
        )
        if query > 0:
            cursor = self.mydb.cursor()
        ...
        query = self.os.system(
            "/etc/init.d/apache2 status | grep -i running 1>/dev/null 2>/dev/null"
        )
        if query > 0:
            cursor = self.mydb.cursor()
        ...
        query = self.os.system(
            "/etc/init.d/mysql status | grep -i running 1>/dev/null 2>/dev/null"
        )
        if query > 0:
            cursor = self.mydb.cursor()
